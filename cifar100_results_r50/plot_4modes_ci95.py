...
MODE_PATTERNS = {
    "geo_r": "results_kfrac_geo_r_cifar100_sd*.yaml",
    "geo_m": "results_kfrac_geo_m_cifar100_sd*.yaml",
    "best_of_random(160)": "results_kfrac_cifar100_bestof_random_sd*.yaml",
    "kwta": "results_kfrac_kwta_cifar100_sd*.yaml",
}
...
def plot_curve_with_band(ax, g_mode, x="k_frac", y_mean="mean", y_lo="ci_lo", y_hi="ci_hi", label=None):
    g_mode = g_mode.sort_values(x)
    ax.plot(g_mode[x], g_mode[y_mean], label=label, linewidth=2)
...
def main():
    ...
    ax.set_title("k_frac vs test_acc (mean, 95% CI band over seeds)")
    ax.grid(True, alpha=0.3)
    ax.legend()
    out1 = os.path.join(args.out_dir, "kfrac_vs_testacc_cifar100.png")
    plt.tight_layout()
    ...
    print("saved:", out2)
    plt.close()
    df.to_csv(os.path.join(args.out_dir, "kfrac_long_cifar100.csv"), index=False)

    g_acc.to_csv(os.path.join(args.out_dir, "kfrac_testacc_cifar100_ci95.csv"), index=False)

    low = df[df["k_frac"] <= 0.2].copy()
    g_t = agg_ci95(low, "elapsed_sec")
    g_t.to_csv(os.path.join(args.out_dir, "kfrac_time_ci95_cifar100.csv"), index=False)
