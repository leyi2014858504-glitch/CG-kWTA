...
def sweep_kfrac_to_yaml(
    ...
    if Shuffle_mode and n_shuffle_runs > 1:
        ...
        for rep in range(n_shuffle_runs):
            rep_seed = shuffle_seed_offset + rep
            rep_path = f"{base}_rep{rep}{ext}"
            print(f"\n===== Shuffle repeat {rep+1}/{n_shuffle_runs} (shuffle_seed={rep_seed}) -> {rep_path} =====")
            _sweep_single(out_path=rep_path, seed=seed, gate_mode=gate_mode,
                          sigma0_override=sigma0_override, maxiter=maxiter,
                          hidden_dim=hidden_dim, ridge_alpha=ridge_alpha,
                          data_dir=data_dir, cma_train_n=cma_train_n,
                          cma_val_n=cma_val_n, dataset=dataset,
                          dataset_download=dataset_download,
                          shuffle_seed=rep_seed)
            results_agg["runs_by_repeat"].append(rep_path)
            results_agg["meta"]["seeds"].append(rep_seed)
        agg_path = f"{base}_multi_shuffle.yaml"
        with open(agg_path, "w", encoding="utf-8") as f:
            yaml.safe_dump(results_agg, f, allow_unicode=True, sort_keys=False)
        print(f"Saved multi-shuffle index: {agg_path}")
        return results_agg
    else:
        return _sweep_single(out_path=out_path, seed=seed, gate_mode=gate_mode,
                             sigma0_override=sigma0_override, maxiter=maxiter,
                             hidden_dim=hidden_dim, ridge_alpha=ridge_alpha,
                             data_dir=data_dir, cma_train_n=cma_train_n,
                             cma_val_n=cma_val_n, dataset=dataset,
                             dataset_download=dataset_download,
                             shuffle_seed=shuffle_seed_offset)
