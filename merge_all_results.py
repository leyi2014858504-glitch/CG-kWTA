...
OUTPUT_FILE = os.path.join(BASE_DIR, "所有实验结果汇总.xlsx")
...
def create_summary_table(all_data):
    ...
    for folder_name, data_dict in all_data.items():
        ...
        summary_rows.append({
            ...
            "平均时间 (s)": avg_time,
            "文件夹": folder_name,
            "模型排序": MODEL_ORDER.get(model, 99),
            "坐标系排序": COORD_ORDER.get(coord, 99),
        })
...
def main():
    ...
    for folder in all_folders:
        ...
        if data:
            ...
            print(f"  ✓ 加载成功 ({len(data)} 个文件)")
        else:
            print(f"  ⚠ 无数据")
    
    print(f"\n成功加载 {len(all_data)} 个实验的数据")
    ...
    # Drop helper columns
    summary_df = summary_df.drop(columns=["模型排序", "坐标系排序"])
    ...
    with pd.ExcelWriter(OUTPUT_FILE, engine="openpyxl") as writer:
        ...
        for dataset in ["cifar10", "cifar100", "stl10"]:
            dataset_df = summary_df[summary_df["数据集"] == dataset].copy()
            dataset_df = dataset_df.drop(columns=["数据集"])
    ...
    print(f"\n包含的工作表:")
    print(f"  - 总览：所有实验的汇总")
    print(f"  - cifar10, cifar100, stl10：按数据集分类")
    print(f"  - 详细数据：每个实验的原始 CSV 数据")
    
    # Show summary statistics
    print(f"\n📈 统计信息:")
    print(f"  总实验数：{len(summary_df)}")
    for dataset in ["cifar10", "cifar100", "stl10"]:
        count = len(summary_df[summary_df["数据集"] == dataset])
        print(f"  {dataset}: {count} 个实验")
