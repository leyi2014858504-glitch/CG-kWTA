...
def infer_seed_from_filename(path: str):
    ...
    for rgx in (RE_SHUFFLE, RE_RANDOM, RE_R50):
        m = rgx.search(base)
        if m:
...
def load_all(data_dir: str, metric: str, nonshuffle_glob: str, shuffle_glob: str, random_glob: str):
    patterns = [
...
def plot_3curves(df: pd.DataFrame, out_png: str, title: str):
    g = (
    ...
    for group, label in [("nonshuffle", "non-shuffle"), ("shuffle", "shuffle"), ("random", "random")]:
        sub = g[g["group"] == group]
        if len(sub) == 0:
            continue
        sub = add_ci_95(sub)
...
def main():
    ...
    ap.add_argument("--out_plot", type=str, default="plot_3curves_kle0p51.png")
    ...
    plot_3curves(df_plot, out_png=args.out_plot, title=title)
