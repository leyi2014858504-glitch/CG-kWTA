...
from shuffle import shuffleptsrowsinplace, fitness_shuffle
...
"cifar10_r50_layer4": dict(inputdim=2048, load=lambda trainsamples, testsamples, datadir, download: loadcachedpt("cifar10_r50_layer4", trainsamples, testsamples, datadir, download)),
    ...
    "cifar10_vitmae": {
        # input_dim 这里填一个占位值，后面 load_dataset() 会用实际 X.shape[1] 覆盖（见下一段改动）
        "input_dim": -1,
        "load": lambda train_samples, test_samples, data_dir, download: load_cifar10_vitmae_from_pt(
            train_samples=train_samples,
            test_samples=test_samples,
            data_dir=data_dir,
            tag="cifar10_vit_base_patch16_224_mae",
...
def sweep_kfrac_to_yaml(
    ...
    "cifar100_r50_layer4b0", "cifar100_r50_layer4b1", "cifar100_r50_layer4b2"}
    for layer_idx in range(VITMAE_NUM_LAYERS):
    ...
    pts_fixed = None
    if gate_mode == "geo" and GEO_PTS_INIT in ["meanvar", "randproj"]:
        set_seed(seed)
        ...
        probe = MainModel(
            ...
            k_frac=0.5,  # doesn't affect backbone init
            gate_mode="geo",
            pts_dim=pd,
        ...
        if GEO_PTS_INIT == "meanvar":
            ...
            print("backbone_type =", backbone_type, "hidden_dim_eff =", hidden_dim_eff)
            print("calib_data =", calib_data.shape, "pts_fixed =", pts_fixed.shape, "probe.backbone =", probe.backbone)
    ...
    k_list = [0.05,0.10,0.20,0.30,0.50]     #The extreme k_frac test
    ...
    for k_frac in k_list:
        ...
        if gate_mode == "geo":
            # (A) CMA fast stage on a subset
            model.set_data(
                train_data_cma.to(device),
                train_labels_cma.to(device),
                val_data_cma.to(device),
                val_labels_cma.to(device),
            )
            optimizer = CmaEngine(model)
            best_params, best_loss, best_acc = optimizer.step(maxiter=maxiter, verbose=False)

            # (B) Full re-evaluation on the full train/val split (warm-start from stage A since geo params were updated)
            model.set_data(
            ...
            N_total = 2 * (popsize * int(maxiter) + 1)
            ...
            # No search: just compute val metrics once (optional), then proceed to final test evaluation.
            model.set_data(
                train_data.to(device),
                train_labels.to(device),
                val_data.to(device),
...
if __name__ == "__main__":
    ...
    MAXITER = 10 #You can set it as "0" if you wanna run random or kwta mode.
    DATASET = "cifar10_r50_layer4b2" #mnist cifar10 cifar10_r50 cifar10_vitmae
    DATA_DIR = "./data"
    # ---------------------------------------------------------------
    ...
    for sd in range(SEED_START, SEED_END + 1):
        ...
        if EXP_MODE == "geo_r":
            ...
            gate_mode = "geo"
            out_path = f"results_ball_sd{sd}.yaml"
            sweep_kfrac_to_yaml(
                out_path=out_path,
                seed=sd,
                gate_mode=gate_mode,
            ...
            gate_mode = "geo"
            out_path = f"results_kfrac_geo_m_cifar100_sd{sd}.yaml"
            sweep_kfrac_to_yaml(
                out_path=out_path,
                seed=sd,
                gate_mode=gate_mode,
            ...
            gate_mode = "kwta"
            out_path = f"results_kfrac_kwta_cifar100_sd{sd}.yaml"
            sweep_kfrac_to_yaml(
                out_path=out_path,
                seed=sd,
                gate_mode=gate_mode,
            ...
            out_path = f"results_kfrac_cifar100_bestof_random_sd{sd}.yaml"
            sweep_kfrac_to_yaml(
                out_path=out_path,
                seed=sd,
                gate_mode=gate_mode,
