...
# 所有参数都在这里调节（无需运行时传参）
# =========================
BASE_DIR = r"C:\Users\20148\PycharmProjects\PythonProject4\stage4_b1-b2"  # 改成你的目录
FILE_PATTERN = "results_kfrac_geo_r_l4b{layer}_shuffle{seed}.yaml"            # 按你的命名规则
...
OUTPUT_PNG = os.path.join(BASE_DIR, "test_acc_bar_by_shuffled_blocks.png")
...
OUTPUT_CSV_RAW = os.path.join(BASE_DIR, "test_acc_by_blocks_shuffle_raw.csv")
OUTPUT_CSV_SUMMARY = os.path.join(BASE_DIR, "test_acc_by_layer_blocks_shuffle_summary.csv")
...
def plot_bar(means, stds, counts):
    ...
    plt.xticks(x, [f"block{l}" for l in LAYERS])
    plt.ylabel("test_acc")
